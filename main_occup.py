...
if __name__ == '__main__':
    ...
    building_params = {                      # keys have to correspond to SimX variable names
        "UseIndividualPresence": True,                                                     # use occupancy profiles
        "PresenceFile": f'"{os.path.join(os.getcwd(), "simx", "occup_schedule.txt")}"',    # path of occupancy profile
        "UseIndividualElecConsumption": True,                                              # use electricity consumption
    ...
    for file in power_files:
        ...
        for val_occup in [True, False]:
            building_params["UseIndividualPresence"] = val_occup
            for val_elec in [True, False]:
                building_params["UseIndividualElecConsumption"] = val_elec
                # Start COM server
                ...
                char = "\\"
                df = simx.save_results_paper(df, object_building, path=os.path.join("result", "paper",
                     f'{path.rsplit(char, 1)[-1].split(".", 1)[0]}_'
                     f'occup_{building_params["UseIndividualPresence"]}_'
                     f'elec_{building_params["UseIndividualElecConsumption"]}.csv'))
