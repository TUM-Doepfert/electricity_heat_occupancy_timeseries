...
/*
 * void CVodeSetUserSerialization
 *
 * Specifies the user-provided functions and data for serialization
 */
	  ...
	  void *userData)
{
  ...
  if (cvode_mem==NULL) {
    CVProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeSetUserSerialization", MSGCV_NO_MEM);
    return(CV_MEM_NULL);
  }

  cv_mem = (CVodeMem) cvode_mem;

  cv_mem->cv_UserSerialization = serializeFun;
  cv_mem->cv_UserDeserialization = deserializeFun;
  cv_mem->cv_UserSerializationSize = serializationSizeFun;
  cv_mem->cv_UserSerializationData = userData;
...
void CVodeSetMaxStepNumberBetweenJacobianEvaluations(void *cvode_mem, int maxStepNumberBetweenJacobianEvaluations)
{
	...
	cv_mem->cv_maxStepNumberBetweenJacobianEvaluations = maxStepNumberBetweenJacobianEvaluations;
}
 ...
 * CVodeGetRootCounter
 *
 * Returns pointer to array showing roots how often a root has switched
 */
...
int CVodeGetRootCounter(void *cvode_mem, int *rootctr)
{
  ...
  for (i=0; i<nrt; i++) rootctr[i] = cv_mem->cv_irootctr[i];
