...
#define _ROOT_COUNTER 0

double _ROOT_PROT_BEGINN[] = {0.0, 35.0, 90.0};
double _ROOT_PROT_ENDN[] = {15.0, 40.0, 100.0};
...
#define ONEONE RCONST(1.1)     /* real 1.1     */
...
/* Changed by ITI (TB): 2008_04_03
#define ETAMX1 RCONST(10000.0)
#define ETAMX2 RCONST(10.0)
#define ETAMX3 RCONST(10.0)
*/
#define ETAMX1 RCONST(5.0)
#define ETAMX2 RCONST(5.0)
#define ETAMX3 RCONST(5.0)
...
static long int RootLocate, iMuller, iRF, iMRF, iWM;
#if defined(__BORLANDC__)
/* DBL_MAX is not a constant expression */
static realtype MinStep = 1.7976931348623157e+308;
#else
static realtype MinStep = DBL_MAX;
#endif
static realtype MaxStep = ZERO;
static realtype TLO;
/*
static int *num_steps_order;
static int max_order;

#ifdef _OPENMP
#pragma omp threadprivate(num_steps_order)
#pragma omp threadprivate(max_order)
#endif
*/
...
void *CVodeCreate(int lmm, int iter, int maxord)
{
  CVodeMem cv_mem;
  ...
  cv_mem->cv_maxStepNumberBetweenJacobianEvaluations = 50;
  ...
  /*changed by ITI TB  March 2009.*/
  cv_mem->cv_rootindex = NULL;
  cv_mem->cv_maxindex = 0;
  cv_mem->cv_signglo = NULL;
  cv_mem->cv_signghi = NULL;
  cv_mem->cv_irootctr = NULL;

  cv_mem->cv_hysteresis = 0;
  cv_mem->cv_epsilon = 1.E-11;

  InitializeTracing(cv_mem);
...
/*changed by ITI TB  March 2009.*/
#define rootindex (cv_mem->cv_rootindex)
#define maxindex (cv_mem->cv_maxindex)

#define signglo (cv_mem->cv_signglo)
#define signghi (cv_mem->cv_signghi)
#define irootctr (cv_mem->cv_irootctr)
#define savtlo (cv_mem->cv_savtlo)
#define savglo (cv_mem->cv_savglo)
#define savthi (cv_mem->cv_savthi)
#define savghi (cv_mem->cv_savghi)
#define savtmid (cv_mem->cv_savtmid)
#define savgmid (cv_mem->cv_savgmid)

#define rootHyst (cv_mem->cv_hysteresis)
#define rootEps (cv_mem->cv_epsilon)
...
int CVodeRootInit(void *cvode_mem, int nrtfn, CVRootFn g, void *gdata)
{
  ...
  if ((nrt != cv_mem->cv_nrtfn) && (cv_mem->cv_nrtfn > 0)) {
    ...
    /*changed by ITI TB  March 2009.*/
    free(rootindex); rootindex = NULL;
    /*free(signglo);*/ signglo = NULL;
    free(signghi); signghi = NULL;
    free(irootctr); irootctr = NULL;
  ...
  /*changed by ITI TB  March 2009.*/
  rootindex = NULL;
  rootindex = (int *) malloc(nrt*sizeof(int));
  ...
  signglo = NULL;
  /*signglo = (realtype*)malloc(nrt*sizeof(realtype));*/
  signghi = NULL;
  signghi = (int*)malloc(nrt*sizeof(int));

  irootctr =  (int *) malloc(nrt*sizeof(int));
          ...
          realtype *tret, int itask)
{
  ...
  /*
  if(num_steps_order == NULL)
  {
    max_order       = cv_mem->cv_qmax_alloc;
    num_steps_order = calloc(max_order,sizeof(int));
  ...
  */
  ...
  if (nst == 0) {
    ...
    /* initialize arrays for tracing of corrector iterations  if not done yet */
    if (cv_mem->cv_trMallocDone == FALSE && tfun)
      InitializeCorrectorTracing(cv_mem);
...
void CVodeFree(void **cvode_mem)
{
  ...
  /*changed by ITI TB  Nov 2012.*/
  /*
  if (num_steps_order) {
    free(num_steps_order); num_steps_order = NULL;
  }
  */

  FreeCorrectorTracing(cv_mem);
...
static int CVStep(CVodeMem cv_mem)
{
  ...
  loop {
	...
	if (kflag == PREDICT_AGAIN && cv_mem->cv_tfun) {
		if (cv_mem->trStates) {
			char msg[256], *msl = NULL;

			sprintf(msg, "ODE solver, predictor corrector iteration, states at t = %.16g:", tn);
			StrPrintHistory(cv_mem, cv_mem->cv_states, tr_states, &msl);
		...
		if (cv_mem->trDer) {
			...
			sprintf(msg, "ODE solver, predictor corrector iteration, derivatives at t = %.16g:", tn);
			StrPrintHistory(cv_mem, cv_mem->cv_derivatives, tr_der, &msl);
			tfun(msg, msl, tf_data);
			free(msl);
		}
		if (cv_mem->trNewtonUpdate) {
			char msg[256], *msl = NULL;

			sprintf(msg, "ODE solver, predictor corrector iteration, Newton updates at t = %.16g:", tn);
			StrPrintHistory(cv_mem, cv_mem->cv_corrections, tr_corr, &msl);
			tfun(msg, msl, tf_data);
			free(msl);
		}
		if (cv_mem->trRes) {
			...
			sprintf(msg, "ODE solver, predictor corrector iteration, residuals at t = %.16g:", tn);
			StrPrintHistory(cv_mem, cv_mem->cv_residuals, tr_res, &msl);
			tfun(msg, msl, tf_data);
			free(msl);
		}
		if (cv_mem->trStepFalse) {
			char msg[256], msl[8192];

			sprintf(msg, "Invalid step (t = %.16g).", tn);
			sprintf(msl,
				"Estimated convergence rate\t%g\n"
				"Current simulation time   \t%.16g\n"
				"Step size                 \t%.16g\n"
				"Method order              \t%d\n",
				crate, tn, h, q);
			tfun(msg, msl, tf_data);
		}
		if (cv_mem->trStatesFalse) {
			char msg[256], *msl = NULL;

			sprintf(msg, "States at t = %.16g:", tn);
			StrPrintHistoryLast(cv_mem, cv_mem->cv_states, tr_states, &msl);
			tfun(msg, msl, tf_data);
			free(msl);
		}
		if (cv_mem->trDerFalse) {
			char msg[256], *msl = NULL;

			sprintf(msg, "Derivatives at t = %.16g:", tn);
			StrPrintHistoryLast(cv_mem, cv_mem->cv_derivatives, tr_der, &msl);
			tfun(msg, msl, tf_data);
			free(msl);
		}
		if (cv_mem->trNewtonUpdateFalse) {
			char msg[256], *msl = NULL;

			sprintf(msg, "Newton updates at t = %.16g:", tn);
			StrPrintHistoryLast(cv_mem, cv_mem->cv_corrections, tr_corr, &msl);
			tfun(msg, msl, tf_data);
			free(msl);
		}
		if (cv_mem->trResFalse) {
			char msg[256], *msl = NULL;

			sprintf(msg, "Residuals at t = %.16g:", tn);
			StrPrintHistoryLast(cv_mem, cv_mem->cv_residuals, tr_res, &msl);
			tfun(msg, msl, tf_data);
			free(msl);
		}
	}
	...
	if (eflag == TRY_AGAIN && cv_mem->cv_tfun) {
		if (cv_mem->trStates) {
			char msg[256], *msl = NULL;

			sprintf(msg, "ODE solver, predictor corrector iteration, states at t = %.16g:", old_t);
			StrPrintHistory(cv_mem, cv_mem->cv_states, tr_states, &msl);
			tfun(msg, msl, tf_data);
			free(msl);
		}
		if (cv_mem->trDer) {
			...
			sprintf(msg, "ODE solver, predictor corrector iteration, derivatives at t = %.16g:", old_t);
			StrPrintHistory(cv_mem, cv_mem->cv_derivatives, tr_der, &msl);
			tfun(msg, msl, tf_data);
			free(msl);
		}
		if (cv_mem->trNewtonUpdate) {
			...
			sprintf(msg, "ODE solver, predictor corrector iteration, Newton updates at t = %.16g:", old_t);
			StrPrintHistory(cv_mem, cv_mem->cv_corrections, tr_corr, &msl);
			tfun(msg, msl, tf_data);
			free(msl);
		}
		if (cv_mem->trRes) {
			...
			sprintf(msg, "ODE solver, predictor corrector iteration, residuals at t = %.16g:", old_t);
			StrPrintHistory(cv_mem, cv_mem->cv_residuals, tr_res, &msl);
			tfun(msg, msl, tf_data);
			free(msl);
		}
		if (cv_mem->trStepFalse) {
			...
			sprintf(msg, "Invalid step (t = %.16g).", old_t);
			sprintf(msl, "Convergence test was succesful but error test failed.\n"
				"Current simulation time\t%.16g\n"
				"Old step size          \t%.16g\n"
				"New step size          \t%.16g\n"
				"Old method order       \t%d\n"
				"New method order       \t%d\n",
				old_t, old_h, h, old_q, q);
			tfun(msg, msl, tf_data);
		}
		if (cv_mem->trStatesFalse) {
			char msg[256], *msl = NULL;

			sprintf(msg, "States at t = %.16g:", old_t);
			StrPrintHistoryLast(cv_mem, cv_mem->cv_states, tr_states, &msl);
			tfun(msg, msl, tf_data);
			free(msl);
		}
		if (cv_mem->trDerFalse) {
			...
			sprintf(msg, "Derivatives at t = %.16g:", old_t);
			StrPrintHistoryLast(cv_mem, cv_mem->cv_derivatives, tr_der, &msl);
			tfun(msg, msl, tf_data);
			free(msl);
		}
		if (cv_mem->trResFalse) {
			...
			sprintf(msg, "Residuals at t = %.16g:", old_t);
			StrPrintHistoryLast(cv_mem, cv_mem->cv_residuals, tr_res, &msl);
			tfun(msg, msl, tf_data);
			free(msl);
		}
  ...
  CVPrepareNextStep(cv_mem, dsm);
	...
	if (cv_mem->cv_tfun) {
		...
		if (cv_mem->trStepTrue) {
			char msg[256], msl[8192];
			sprintf(msg, "Valid step (t = %.16g).", old_t);
			sprintf(msl,
				"Current simulation time\t%.16g\n"
				"Old step size          \t%.16g\n"
				"New step size          \t%.16g\n"
				"Old method order       \t%d\n"
				"New method order       \t%d\n",
				old_t, old_h, hprime, old_q, qprime);
			tfun(msg, msl, tf_data);
		}
		if (cv_mem->trStatesTrue) {
			char msg[256], *msl = NULL;

			sprintf(msg, "Corrector, states at t = %.16g:", old_t);
			StrPrintHistoryLast(cv_mem, cv_mem->cv_states, tr_states, &msl);
			tfun(msg, msl, tf_data);
			free(msl);
		}
		if (cv_mem->trDerTrue) {
			...
			sprintf(msg, "Corrector, derivatives at t = %.16g:", old_t);
			StrPrintHistoryLast(cv_mem, cv_mem->cv_derivatives, tr_der, &msl);
			tfun(msg, msl, tf_data);
			free(msl);
		}
		if (cv_mem->trResTrue) {
			...
			sprintf(msg, "Corrector, residuals at t = %.16g:", old_t);
			StrPrintHistoryLast(cv_mem, cv_mem->cv_residuals, tr_res, &msl);
			tfun(msg, msl, tf_data);
			free(msl);
		}
...
static int CVNewtonIteration(CVodeMem cv_mem)
{
  ...
  loop {
    ...
    if (cv_mem->cv_tfun) {
      memcpy(cv_mem->cv_states + m * cv_mem->cv_dim, NV_DATA_S(y), cv_mem->cv_dim * sizeof(realtype));
      memcpy(cv_mem->cv_derivatives + m * cv_mem->cv_dim, NV_DATA_S(ftemp), cv_mem->cv_dim * sizeof(realtype));
      memcpy(cv_mem->cv_residuals + m * cv_mem->cv_dim, NV_DATA_S(tempv), cv_mem->cv_dim * sizeof(realtype));
    }
    ...
    if (cv_mem->cv_tfun)
      memcpy(cv_mem->cv_corrections + m * cv_mem->cv_dim, NV_DATA_S(b), cv_mem->cv_dim * sizeof(realtype));
    ...
    /*Changed by ITI (TB): 2006_09_08*/
    /*set b[j] to 0 if xdot[j]=0*/
    {
      int j, k, isZero;
      long n, m;
      realtype* pb, *pftemp;
      ...
      pftemp = N_VGetArrayPointer(ftemp);
      pb = N_VGetArrayPointer(b);

      for (j = 0; j < n; j++)
      {
        if((pftemp[j] == ZERO) && (ABS(pb[j]) < cv_mem->cv_uround))
        {
          /*necessary?*/
          isZero = 1;
          for (k = 1; k <= q; k++)
            if(N_VGetArrayPointer(zn[k])[j] != ZERO)
            {
              isZero = 0;
              break;
            }
    ...
    if (dcon <= ONE) {
      ...
      if(dsm > ONE && dsm < THRESH && !damp) {
		...
		if (cv_mem->cv_tfun)
			memcpy(cv_mem->cv_states + (m + 1) * cv_mem->cv_dim, NV_DATA_S(y), cv_mem->cv_dim * sizeof(realtype));
    ...
    if ((m >= maxcor) || ((m >= 2) && (del > RDIV*delp))) {
      ...
      /* Max. two additional iterations if 1.0<dcon<1.1 */
      if(!(dcon < ONEONE && m<5)) {
        damp = FALSE;
		--mnewt; /* ITI: correction for tracing */
        if ((!jcur) && (setupNonNull)) return(TRY_AGAIN);
        else                           return(CONV_FAIL);
      }
    }
 ...
 * In this case, if ncf is greater or equal than maxncf and |h| < 1.E-010
 * or |h| < hmin, we return the value CV_CONV_FAILURE (if nflag=CONV_FAIL)
 * or CV_REPTD_RHSFUNC_ERR (if nflag=RHSFUNC_RECVR).
                         ...
                         int *ncfPtr)
{
  ...
  /* If we had at least maxncf failures and h < 1.E-010 or |h| < hmin,
     return CV_CONV_FAILURE or CV_REPTD_RHSFUNC_ERR. */

  if ((ABS(h) <= hmin*ONEPSM) || (*ncfPtr >= maxncf && ABS(h) < TINY)) {
  ...
  /* Changed by ITI (TB): 2010_09_22*/
  if(nflag == RHSFUNC_RECVR)
  {
    if((eta*h + tretlast) == tretlast)
 ...
 *
 *   - if at least maxnef error test failures have occurred and |h| < 1.E-010
 *     or if ABS(h) < hmin, we return CV_ERR_FAILURE.
 *
                                 ...
                                 realtype saved_t, int *nefPtr, realtype *dsmPtr)
{
  ...
  *nflagPtr = PREV_ERR_FAIL;
  ...
  /* At least maxnef failures and |h| < 1.E-010 or |h| < hmin, return CV_ERR_FAILURE */
  if ((ABS(h) <= hmin*ONEPSM) || (*nefPtr >= maxnef && ABS(h) < TINY))
...
static int CVRcheck1(CVodeMem cv_mem)
{
  ...
  tlo = tn;
  test = ABS(tn) + ABS(h);
  ttol = (_nextafter(test,DBL_MAX) - test)*10.0;

  /* Evaluate g at initial t and check for zero values. */
  retval = gfun(tlo, zn[0], glo, g_data, 0);

  /* Changed by ITI (TB): 2011_06_20, Update signglo (old_sign_zf) to ensure consistent values for zfs of delay buffers*/
  for (i = 0; i < nrtfn; i++) {
    if(glo[i] > ZERO)
      signglo[i] = 1;
    else if(glo[i] < 0)
...
static int CVRcheck2(CVodeMem cv_mem)
{
  ...
  retval = gfun(tlo, y, glo, g_data, 0);
  nge++;
  if (retval != 0) return(CV_RTFUNC_FAIL);
...
static int CVRcheck3(CVodeMem cv_mem)
{
  ...
  retval = gfun(thi, y, ghi, g_data, 0);
  nge++;
  if (retval != 0) return(CV_RTFUNC_FAIL);
 ...
 * ComputeRootValues
 *
 * This function computes all the root values at a time ttest.
 * The state variable values at this time are calculated
 * and stored in y. The g values are then calculated and
 * stored in gtest
 *
 * Constructed by ITI e  January 2009.
 *
 */

static void ComputeRootValues(CVodeMem cv_mem, realtype ttest, realtype *gtest)

{
  (void) CVodeGetDky(cv_mem, ttest, 0, y);

  (void) gfun(ttest, y, gtest, g_data, 0);

  nge++;
}

/*
 * CheckRootsInInterval
 *
 * The interval is tlo <= t < thi. There are nrtfn root functions
 * and they are all checked to see if there is a root. For there
 * to be a root, one of the following must be true:
 *
 *    glo[i] == 0 and ghi[i] != 0
 *    glo[i] > 0  and ghi[i] < 0
 *    glo[i] < 0  and ghi[i] > 0
 *
 * For all root functions that have a root iroots[i] is set to 1
 * else it is set to 0. The variable imax is set to the root estimated
 * to be the most extreme and imax is the return value. If there are
 * no roots, -1 is returned.
 *
 * Note that using linear interpolation each root function that crosses
 * the axis does so at a t value
 *
 *  thi - (thi - tlo).ghi/(ghi - glo)
 *
 * Hence the left-most one is the one for which ghi/(ghi - glo)
 * is greatest.
 *
 * Constructed by ITI e  January 2009.
 *
...
static int CheckRootsInInterval(CVodeMem cv_mem, int iReBuild)

{
  int i, imax, iCnt = 0, index=0, tmpmax=0;
  realtype maxfrac, gfrac;
  ...
  for (i = 0;  i < tmpmax; i++)
  {
    ...
    {
      index = i;
      /*TB: 2009_08_10 Save sign of glo*/
      /*TB: 2009_08_26 Save sign of ghi*/
      /*
      if(glo[index] > ZERO)
        signglo[index] = +1.0;
      else if(glo[index] < ZERO)
        signglo[index] = -1.0;
      else
        signglo[index] = ZERO;
      */
    ...
    if (ABS(glo[index]) == ZERO)
    {
      if(ABS(ghi[index]) == ZERO)
      {
        /* This counts as no root. It could be a root in the next step. */

        iroots[index] = 0;
      }
      else if(
        ((signglo[index] == ZERO) && (ghi[index] > ZERO) && (signghi[index] > ZERO))
        || ((signglo[index] == ZERO) && (ghi[index] < ZERO) && (signghi[index] < ZERO))
        || ((signglo[index]<0) && (ghi[index] > ZERO))
        || ((signglo[index]>0) && (ghi[index] < ZERO))
      ) /*TB: 2009_08_10 check if really a sign change happened */
      {
#ifdef __PREVENT_DOUBLEFIND
        /* This is a root at tlo. */

        if(irfnd && tlo == TLO)
        {
          /* The root was at the end of the previous step.
             There must have been an initialization and so
          this does not count as a root to be located. */
        ...
        {
          iroots[index] = 1;
          gfrac     = maxfrac = ONE; /* This cannot be exceeded. */
          imax      = index;
          if(iReBuild)
          {
            rootindex[iCnt] = index;
            iCnt++;
          }
        }
      }
    ...
    {
      /* Here glo[i] is not zero. ghi[i] may be zero. */
      ...
      else if ((glo[index] > ZERO && ghi[index] < ZERO) ||(glo[index] < ZERO && ghi[index] > ZERO))
      {
        /* Definitely a root. */

        iroots[index] = 1;
        gfrac = ABS(ghi[index]/(ghi[index] - glo[index])); /* Less than 1. */
        if (gfrac > maxfrac)
        {
          maxfrac = gfrac;
          imax    = index;
 ...
 * IsRoot
 *
 * TB: 2009_08_10: Checks if we really have a signchange which signals an event
 *
 *
 *
 */
static int IsRoot(realtype _signglo, realtype _signghi, realtype _glo, realtype _ghi)
{
  if((_glo < ZERO && _ghi > ZERO) ||
      (_glo > ZERO && _ghi < ZERO))
  {
    return 1;
  }
    ...
    (ABS(_glo) == ZERO) && (
      ((_signglo == ZERO) && (_ghi > ZERO) && (_signghi > ZERO)) ||
      ((_signglo == ZERO) && (_ghi < ZERO) && (_signghi < ZERO)) ||
      ((_signglo<0) && (_ghi > ZERO)) ||
      ((_signglo>0) && (_ghi < ZERO)))
  )
  {
    return 1;
 ...
 * UseNewtmid
 *
 * Either tlo or thi must be replaced by tmid. This is done so that
 * tlo<thi and there is a proper sign change.
 *
 *
 * Constructed by ITI e  January 2009.
 *
...
static void UseNewtmid(CVodeMem cv_mem, realtype tmid)

{
  int i, index=0, left = 0;

  for(i = 0; i < maxindex; i++) {
    index = rootindex[i];
    if(IsRoot(signglo[index], signghi[index], glo[index], grout[index]))
    {
      left = 1;
      break;
    }
 ...
 * GeneralRebracket
 *
 * All the methods we use are bracketing methods with the root
 * between tlo and thi. A new estimate has been calculated tmid.
 * Either tlo or thi must be replaced by tmid.
 *
 * The variable side must also be reset and, if necessary imax
 * must be updated.
 *
 * Constructed by ITI e  January 2009.
 *
...
static void GeneralRebracket(CVodeMem cv_mem, realtype tmid, int *imax, int *side)

{
  UseNewtmid(cv_mem, tmid);
  ...
  /* It may be approriate to change *imax. */

  *imax = CheckRootsInInterval(cv_mem, 0);
}
 ...
 * ReassignOldValues
 *
 * Since the start of the previous iteration one or both of tlo, thi
 * will have changed. We must reset some saved values to get reasonably
 * spaced savtlo < savtmid < savthi.
 *
 * Constructed by ITI e  January 2009.
 *
 */

static void ReassignOldValues(CVodeMem cv_mem, int imax)

{
  if(savtmid > tlo && savtmid < thi)
  {
    /* This is perfect but actually I do not think it can happen. */
 ...
 * one exists. The method is very good for linear root functions
 * and given exact arithmetic gets the crossing point exactly in
 * 1 iteration. With real computer arithmetic  it may be an atomic
 *  distance to the left with glo very small but non-zero. It such a
 * situation in the next iteration *tmid may well be calculated as
 * the same as tlo. To get out of this infinite loop pushing *tmid
 * an atomic distance to the right cures things in 1 or 2 iterations.
 * I also tried switching to WM in these circumstances --- it worked
 * but wanted a lot more iterations (typically 18).
 *
 * For the first two iterations the method is linear interpolation.
 * After this, if the method becomes one sided, the value of alpha
 * is used to discourage more tlo updates (alpha>1) or discourage
 * more thi updates (alpha<1).
 *
 * Return values/
 *
 *     1 good estimate ontained
 *     0 glo=0 switch to MRF
 *    -1 a good estimate but at the atomic level.
 *       RF can continue but Muller must switch to RF.
 *
 * Constructed by ITI e  January 2009.
 *
 */

static int DoRegulaFalsi(CVodeMem cv_mem, realtype *tmid, realtype alpha,
                         int imax)

{
  /* Deal with special case glo=0. We will switch to MRF method. */

  if(glo[imax] == ZERO)
  {
    return 0;
  }
  /* Set next root approximation tmid. */

  *tmid = thi - (thi - tlo)*ghi[imax]/(ghi[imax] - alpha*glo[imax]);

  iRF++;

  /* The first of the following does happen reasonably frequently.
     Usually the new value coincides with tlo which has a glo
     incredibly close to zero. This happens with linear root functions
     when with exact arithmetic the g-value would have been zero.
     Normally 1 or 2 smallest possible pushes to the left is all
     that is necessary. THIS CODE IS DELICATE! IF YOU CHANGE IT
     BE VERY CAREFUL. */

  if(*tmid <= tlo) /* Usually it will be equal and usually
                      glo[imax] is very nearly zero. */
  {
    /* Push a smallest amount possible to the right. */

    *tmid = _nextafter(tlo,DBL_MAX);

    /* This is actually a good return for RF but is
       a bad return for Muller which calls RF. Muller
       must switch to a proper RF. */

    return -1;
  }
  else if(*tmid >= thi) /* I have actually never seen this happen
                 but it might do. There would probably be
               equality with ghi[imax] very nearly zero. */
  {
    /* Push a smallest amount possible to the left. */

    *tmid = -_nextafter(-thi,DBL_MAX);
 ...
 *
 * This routine solves for a root of g(t) between tlo and thi, if
 * one exists. The method is very good for non-linear root functions
 * and hence is a perfect complement for Regula Falsi. To be robust
 * it must be implemented as a bracketing method.
 *
 * Quadratic interpolation is used and hence three points are
 * required. At the first iteration an application of Regula
 * Falsi supplies the third point. After this previous values
 * can be used.
 *
 * The method is very delicate to code and a number of
 * pathelogical cases must be must be allowed for:
 *
 *      The three points are (almost) colinear and we must switch to
 *      to Regula Falsi.
 *      The value of glo is zero and we switch to modified Regula
 *      Falsi.
 *      Near convergence the roots are complex and we switch to
 *      Regula Falsi.
 *      Near covergence the roots are real but not in the correct
 *      interval and we switch to Regula Falsi.
 *
 * Return values:
 *
 *   1 a suitable root has been found
 *   0 either glo was 0 or the Regula Falsi method produced a time
 *     value with a 0 root function value. Signal a switch to the
 *     modified Regula Falsi method
 *  -1 points (almost) colinear, complex roots or both roots outside
 *     valid range. Switch to Regula Falsi.
 *
 * Constructed by ITI e  January 2009.
 *
 */

static int DoMuller(CVodeMem cv_mem, realtype *tmid, int imax, int first)
{
  realtype root, grad1, grad2, dmidlo, dmidhi, dlohi, dmidlohi, w,
           numer, denom, test1, test2;
  int ret_value, oldimax, side;

  if(glo[imax] == ZERO)
  {
    /* glo is zero and we are wasting our time here. Get out quick. */

    return 0;
  }
  if(first)
  {
    /* We need three points and we have only two.
       Use Regula Falsi estimate to get the third point. */

    ret_value = DoRegulaFalsi(cv_mem, tmid, ONE, imax);

    if(ret_value != 1)
    {
      return ret_value;
    }
    /* The savtmid root values will be stored in grout. */

    ComputeRootValues(cv_mem, *tmid, grout);

    /* Save strategic values. */
    ...
    /* I use the mid values. Either (tlo,glo) or (thi, ghi)
       will get replaced by (tmid, grout). (thi - tlo) will
     be reduced in size. */

    oldimax = imax;
    GeneralRebracket(cv_mem, *tmid, &imax, &side);
    if(imax != oldimax)
      return ret_value;
  }
  else
  {
    ReassignOldValues(cv_mem, imax);
  }
  /* Check for (almost) linearity.*/

  grad1 = (savgmid - savglo)/(savtmid - savtlo);
  grad2 = (savghi - savgmid)/(savthi - savtmid);

  if(ABS(grad1 - grad2) < 1.0e-6*MAX(ABS(grad1),ABS(grad2)))
  {
    /* It seems very linear. Switch to Regula Falsi. */

    return -1;
  }
  /* We do not have a zero glo and it is not (almost) linear.
     So we go ahead with Muller. Compute divided differences. */

  iMuller++;

  dmidlo = (savgmid - savglo)/(savtmid - savtlo);
  dmidhi = (savgmid - savghi)/(savtmid - savthi);
  dlohi  = (savglo - savghi)/(savtlo - savthi);

  dmidlohi = (dlohi - dmidlo)/(savthi - savtmid);

  w = dmidlo + dmidhi - dlohi;

  root = w*w - 4.0*savgmid*dmidlohi;

  if(root < ZERO)
  {
    /* Complex root. Can happen very close to convergence.
       Get out quick. */
    ...
    if(ret_value == 1)
    {
      /* A good RF value but not good Muller value.
         Signal a switch to Regula Falsi. */
  ...
  {
    /* Real roots. Compute root with positive sign. */
    ...
    if(denom != ZERO)
    {
      test1 = savtmid - numer/denom;

      if(test1 > tlo && test1 < thi)
      {
        /* We have found a suitable root. */
    ...
    /* Try the other root (with negative sign). */

    denom = w - root;

    if(denom != ZERO)
    {
      test2 = savtmid - numer/denom;

      if(test2 > tlo && test2 < thi)
      {
        /* We have found a suitable root. */
  ...
  /* If we get here we have not got a satisfactory root. We have set a
     default *tmid and we do a error return with this value. */
 ...
 *
 * This routine solves for a root of g(t) between tlo and thi, if
 * one exists. The method is useful for cases in which glo is zero.
 *
 * The smallest time value for which the root function value is
 * non-zero makes the perfect value for thi. However, this not always
 * the value given by nextafter. The stopping tolerance is thi-tlo < ttol.
 * I try the point distance ttol*0.9 to the left of tlo.
 *
 * Constructed by ITI e  January 2009.
 *
 */

static void DoModRegulaFalsi(CVodeMem cv_mem, realtype *tmid)
{
  iMRF++;
 ...
 *
 * This routine solves for a root of g(t) between tlo and thi, if
 * one exists. The method does not look at the magnitude of the root
 * function values but only the sign. This makes it suitable for
 * very sick root functions. The method is a generalization of the
 * bisection method. If alpha=1, it is the bisection method.
 *
 * Note that imax is the root function that is currently under consideration.
 *
 * Constructed by ITI e  January 2009.
 *
 */

static void DoWeightedMean(CVodeMem cv_mem, realtype *tmid, realtype alpha)

{
  iWM++;

  /* Set next root approximation tmid. Maybe this formulation
     minimizes errors. */

  if(alpha >= ONE)
  {
    *tmid = tlo + alpha*(thi - tlo)/(alpha + ONE);
  }
  else
  {
    *tmid = thi - (thi - tlo)/(alpha + ONE);
  }
  /* The following do happen reasonably frequently. The
     new value coincides with an end point. If no action
     is taken, there may be useless RF iterations until alpha
     becomes big enough or small enough to move *tmid away
     from the end point. */
 ...
 *
 * The methods used are Muller's method, the Regua Falsi method,
 * a special method for cases for which g(tlo) is zero and a weighted
 * method. The weighted mean method does not look at the magnitude of
 * g but only the signs of glo and ghi. It is designed for very sick
...
/* Changed by ITI (TB): 2006_09_07*/
/* TB: 2006_09_07: roots must have moved away from zero according to MODELICA Spec*/
/* Extra methods added January 2009. */

static int CVRootfind(CVodeMem cv_mem)
{
  realtype alpha, tmid;
  int i, imax, oldimax, side, sideprev, method, root_rtn, first, oldnge, iterations;

#define MULLER         0
#define REGULAFALSI    1
#define MODREGULAFALSI 2
#define WEIGHTEDMEAN   3

  imax = 0;

  /* Preserve the tlo corresponding to the integrator value. */

  TLO = tlo;


  /*  if(thi>=0.0003514717303118234) */
  /*    printf("now\n"); */

  /* First check for change in sign in ghi or for a zero in ghi. */
  imax = CheckRootsInInterval(cv_mem, 1);

  if(imax == -1)
  {
    /* No root was found in the interval, reset trout and grout.
       Then return CV_SUCCESS to indicate integrator may continue
     to the next step.  */
  ...
  if(tlo == thi)
  {
    ...
    for (i = 0; i < nrtfn; i++)
    {
      ...
      if ((ABS(ghi[i]) != ZERO) && (glo[i]*ghi[i] <= ZERO)) iroots[i] = 1;
    }
    return(RTFOUND);
  }
  ...
  /* At least one root was found.  Loop to locate leftmost root. */
  ...
  loop {    /* Looping point */
    ...
    if(method == MULLER)
    {
      root_rtn = DoMuller(cv_mem, &tmid, imax, first);

      if(root_rtn == 0)
      {
        /* glo is zero. Use Modified Regula Falsi. */

        method = MODREGULAFALSI;

        continue;
      }
      else if(root_rtn == -1)
      {
        /* Either (almost) linear or too close to convergence
        for Muller to work properly or even Regula Falsi
        at atomic level. */
    ...
    {
      root_rtn = DoRegulaFalsi(cv_mem, &tmid, alpha, imax);
    ...
    else if(method == MODREGULAFALSI)
    {
      DoModRegulaFalsi(cv_mem, &tmid);
    }
    else if(method == WEIGHTEDMEAN)
    {
      /* The only way we get here is if 8 iterations fail
         to achieve convergence. */

      DoWeightedMean(cv_mem, &tmid, alpha);
    }
    first = 0;
    ComputeRootValues(cv_mem, tmid, grout);

    /* Check to see in which subinterval g changes sign, and reset imax.
         Set side = 1 if root is on low side, or 2 if on high side.  */
    ...
    if(ABS(thi - tlo) <= ttol) /* Stop at root thi if convergeed, otherwise loop on. */
    {
      gfun(0, 0, 0, g_data, 2);
      break;
    }
    else if(imax != oldimax)
    {
      /* This can happen. Treat as a new problem. */
    ...
    if(iterations == 8)
    {
      /* If it is a linear root function, it should converge
         in 2 or 3 iterations. If it is non-linear, it should
         converge in 3 or 4 and certainly 8 or fewer iterations.
         If we reach this point it is probably sick. I assume that
         the magnitude of the root function values cannot be relied
         upon. Use a method that only looks at the sign. */
    ...
    else if(method == REGULAFALSI || method == WEIGHTEDMEAN)
    {
      /* Set weight alpha.
         ...
         The next guess tmid is the standard method value if alpha = 1, but
  ...
  for(i = 0; i < maxindex; i++)
  {
    ...
    if(IsRoot(signglo[index], signghi[index], glo[index], ghi[index]))
    {
      iroots[index] = 1;
      ...
      if(((cv_mem->cv_tn >= _ROOT_PROT_BEGINN[0])&&(cv_mem->cv_tn <=_ROOT_PROT_ENDN[0]))||((cv_mem->cv_tn >=_ROOT_PROT_BEGINN[1])&&(cv_mem->cv_tn <=_ROOT_PROT_ENDN[1]))||((cv_mem->cv_tn >=_ROOT_PROT_BEGINN[2])&&(cv_mem->cv_tn <=_ROOT_PROT_ENDN[2])))
        irootctr[index]++;
    }
  }
  return (RTFOUND);
...

{
  ...
  {
    printf("\tOrder %d steps   %d\n",i+1, num_steps_order[i]);
  }
  printf("\tSmallest step %.4e\n", MinStep);
  printf("\tLargest step %.4e\n", MaxStep);
  printf("\tMuller %ld Regula Falsi %ld Modified RF %ld Weighted Mean %ld\n",
    iMuller, iRF, iMRF, iWM);
...
void CVodeSetSignGloData(void *cvode_mem, int* v)
{
  CVodeMem cv_mem = (CVodeMem) cvode_mem;
  signglo = v;
}

void CVodeUpdateGlo(void *cvode_mem, realtype* v)
{
  CVodeMem cv_mem = (CVodeMem) cvode_mem;
  size_t i;

  for(i=0; i<(size_t)nrtfn; i++)
    glo[i] = v[i];
}

void CVodeSetRootHysteresis(void *cvode_mem, int v)
{
  CVodeMem cv_mem = (CVodeMem) cvode_mem;
  rootHyst = v;
}

void CVodeSetRootEpsilon(void *cvode_mem, realtype v)
{
  CVodeMem cv_mem = (CVodeMem) cvode_mem;
  rootEps = ABS(v);
}
