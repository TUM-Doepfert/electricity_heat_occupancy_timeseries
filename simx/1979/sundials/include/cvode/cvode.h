 ...
 * Type : CVTraceFn
 * -----------------------------------------------------------------
 * A function which handles tracing messages must have type
 * CVTraceFn.
 * The function takes as input the tracing message and a pointer to
 * user data, the same as that passed to CVodeSetTraceFn.
 *
 * A CVTraceFn has no return value.
 * -----------------------------------------------------------------
 */

typedef void (*CVTraceFn)(char *msg, char *msl, void *tf_data);

/*
 * -----------------------------------------------------------------
 * Type : CVSerializationFn
 * -----------------------------------------------------------------
 * A serialization or deserialization function must have type
 * CVSerializationFn.
 * The function takes as input a pointer to the data that has to be
 * serialized and to the memory block where the data is stored.
 * 
 * It returns the size of the memory used.
 * -----------------------------------------------------------------
 */
typedef long (*CVSerializationFn)(void *serData,
                                  void **storageDataPtr);

/*
 * -----------------------------------------------------------------
 * Type : CVSerializationSizeFn
 * -----------------------------------------------------------------
 * A function that returns the size of the memory needed for
 * serialization must have type CVSerializationSizeFn.
 * The function takes as input a pointer to the data that has to be
 * serialized.
 * 
 * It returns the size of the memory needed.
 * -----------------------------------------------------------------
 */
typedef long (*CVSerializationSizeFn)(void *serData);
 ...
 *                         | [NULL]
 *                         |
 *CVodeSetUserSerialization| user-provided functions for serialization,
 *                         | deserialization, and the size of memory
...
int CVodeSetUserSerialization(void *cvode_mem, CVSerializationFn serializeFun,
    CVSerializationFn deserializeFun, CVSerializationSizeFn serializationSizeFun,
	void *userData);
...
void CVodeSetMaxStepNumberBetweenJacobianEvaluations(void *cvode_mem, int maxStepNumberBetweenJacobianEvaluations);
...
int CVodeGetRootCounter(void *cvode_mem, int *rootctr);
