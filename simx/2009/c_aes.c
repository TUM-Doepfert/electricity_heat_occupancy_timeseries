...
static const byte expkey[] = {
    
};
...
/* http://en.wikipedia.org/wiki/Advanced_Encryption_Standard */
static void EncryptAES(const byte *msg, const byte *keys, byte *c) {
...
/* http://en.wikipedia.org/wiki/Advanced_Encryption_Standard */
static void DecryptAES(const byte *c, const byte *keys, byte *m) {
    int i; /* To count the rounds */
...
int decryptLine(char** buf, int* bufLen, FILE* fp, size_t* offset) {
    int ch, idx = 0;
    byte in[16] = {0};
    char* p;
    ...
    if (*bufLen <= 16) {
        fclose(fp);
        free(*buf);
        ModelicaError("Read buffer size is too small.\n");
        return 1;
    }
    ...
    if (*offset >= (size_t)*bufLen - 1) {
        fclose(fp);
        free(*buf);
        ModelicaError("Invalid offset for read buffer.\n");
        return 1;
    }

    {
        size_t len = (size_t)*bufLen - (*offset + 1);
        if ((p = (char*)memchr(&((*buf)[*offset + 1]), '\0', len)) != NULL) {
            len = (size_t)(p - &((*buf)[*offset + 1]));
        }
        memmove(*buf, &((*buf)[*offset + 1]), (size_t)*bufLen - (*offset + 1));
        memset(&((*buf)[len]), 0x00, (size_t)*bufLen - len);
    ...
    while ((ch = getc(fp)) != EOF) {
        ...
        while (*offset + 16 > (size_t)*bufLen) {
            ...
            *buf = tmp;
            memset(&((*buf)[oldBufLen]), 0x00, (size_t)oldBufLen);
        }

        DecryptAES(in, expkey, (byte*)(&((*buf)[*offset])));
        *offset += 16;
        if ((p = (char*)memchr(*buf, '\n', *offset)) != NULL) {
            *p = '\0';
            *offset = (size_t)(p - (*buf));
